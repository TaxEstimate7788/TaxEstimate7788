/*------------------------------------------------------------------------------
* NAME       : implicitfunctionExample1
* PURPOSE    : Inputs example of implicit function  
* PARAMETERS :  
* RETURNS    :  
*----------------------------------------------------------------------------*/
    function implicitfunctionExample1(){
        document.getElementById("implicitfunction1").value = 'yÂ³ + yÂ² -5y -xÂ² = -4';
        //GraphingCalculator33();
    }
    
/*------------------------------------------------------------------------------
* NAME       :  DrawTangentLine()
* PURPOSE    :  Draws tangent line 
* PARAMETERS :  equation of function, y prime, tangent point(s)
* RETURNS    :  graphs of equation and tangent line
*----------------------------------------------------------------------------*/
...
*----------------------------------------------------------------------------*/
    function DrawTangentLine1(){
        ...
        document.getElementById("TextBoxForCartesianGraph1").value = '36+yÂ²';
...
/*------------------------------------------------------------------------------
* NAME       : SolveEquationNewtonRaphson  
* PURPOSE    : Finds approximate solution to equation  
* PARAMETERS : equation,  leftbound, rightbound
* RETURNS    : approximate numerical solution 
*----------------------------------------------------------------------------*/
...
* RETURNS    :  result of polynomial addition
*----------------------------------------------------------------------------*/
 ...
 * PURPOSE    : Processes input string with characters "{ }" ; "{ }" is used for restriction on 'x'.
 * PARAMETERS : string with characters "{ }"
 * RETURNS    : String with "{a<=x<=b}" will be converted to "INQUALITYG(a;x;b)".
                String with "{a<=x<b}"  will be converted to "INQUALITYH(a;x;b)".
                String with "{a<x<=b}"  will be converted to "INQUALITYI(a;x;b)".
                String with "{a<x<b}"   will be converted to "INQUALITYJ("a;x;b)".
                String with "{x<=a}"    will be converted to "INQUALITYA(x;a)".
                String with "{x<a}"     will be converted to "INQUALITYB(x;a)".
                String with "{x>=a}"    will be converted to "INQUALITYC(x;a)".
                String with "{x>a}"     will be converted to "INQUALITYD(x;a)".
                String with "{x!=a}"    will be converted to "INQUALITYDD(x;a)". 
                String with "{x=a}"     will be converted to "INQUALITYF(x;a)". 
 *----------------------------------------------------------------------------*/
function RestrictionType(str) {
    ...
    if (str1 == '{' && str2 == '<' && str3 == '=') {
        var nn1 = str.indexOf('<=');
        var tt = str.substring(nn1);
        tt = tt.replace('}', '');
        tt = tt.replace('<=', '');
        var tt1 = FindValue(tt);
        str = str.replace(tt , tt1);
        ...
        bb = gg[1].replace('}', '');
        bb = bb.replace('<=', '');
        var tt2 = FindValue(bb);
        str = str.replace(tt , tt1);
        str = str.replace(bb , tt2);
        //alert('tt1 =' + tt1);
...
function ProcessInequality(expr) {
    var EquNum = 0;
    for (var i = 1; i <= 20; i++) {
        if( expr == document.getElementById('TextBoxForCartesianGraph'+ i).value){
             EquNum = i;
             break;
         };
    }
        ...
        expr = replaceAll(expr, 'y', 'x');//added on 26Oct2016

        if (expr.indexOf('!=') > -1) {
            var tt = expr.split('{');
            var equ = tt[0];
            var hh = tt[1].split('!=');
            var val = hh[1].replace('}', '');

            //alert('equ =' + equ + '........val = ' + val); 
            document.getElementById("Point" + EquNum + "X").value = val;
            var val2 = FindValue(val);
            var equ = replaceAll(equ, 'x', '(' + val2 +')' )
            document.getElementById("Point" + EquNum + "Y").value = FindValue(equ);
            document.getElementById("Point" + EquNum + "statusopen").checked = true;
            //document.getElementById("Point" + i + "statusclose").checked = false;
            document.getElementById('checkboxCartesianPoints').checked = true;
        ...
        if (expr.indexOf('{x=') > -1) {
            ...
            var equ = replaceAll(equ, 'x', '(' + val2 +')' )
            document.getElementById("Point" + EquNum + "Y").value = FindValue(equ);
            //document.getElementById("Point" + i + "statusopen").checked = false;
            document.getElementById("Point" + EquNum + "statusclose").checked = true;
            document.getElementById('checkboxCartesianPoints').checked = true;
            //document.getElementById("pointsizeL").checked = true;
 ...
 * PURPOSE    : Converts two-variable linear equation in general form to slope-intercept form
 * PARAMETERS : linear equation in general form 
 * RETURNS    : linear equation in slope intercept form
 *----------------------------------------------------------------------------*/
...
function ProcessInput(StringToProcess) {
        ...
        ggg = replaceAll(ggg, 'root', "Root");
        ggg = replaceAll(ggg, 'ROOT', "Root");
        ggg = replaceAll(ggg, 'Root2', "SQRT");
        ggg = replaceAll(ggg, 'Root3', "CBROOT");
        ggg = replaceAll(ggg, 'Root4', "FOURTHROOT");
        ggg = replaceAll(ggg, 'Root5', "FIFTHROOT");
        ggg = replaceAll(ggg, 'Root6', "SIXTHROOT");
        ggg = replaceAll(ggg, 'Root7', "SEVENTHROOT");
        ggg = replaceAll(ggg, 'Root8', "EIGTHROOT");
        ggg = replaceAll(ggg, 'Root9', "NINTHROOT");
        ggg = replaceAll(ggg, 'Root10', "TENTHROOT");
...
/*------------------------------------------------------------------------------
 * NAME       : replaceAll
 * PURPOSE    : Replaces string with new string
 * PARAMETERS : string
 * RETURNS    : string
 *----------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
 * NAME       : isInteger
 * PURPOSE    : Determines if a string is an integer
 * PARAMETERS : string
 * RETURNS    : boolean
 *----------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
 * NAME       : checkineq
 * PURPOSE    : Determines if inequality is true or false
...
/*------------------------------------------------------------------------------
 * NAME       : dotproduct
 * PURPOSE    : Calculates dot product of two vectors
...
/*------------------------------------------------------------------------------
 * NAME       : SolveEquation3
 * PURPOSE    : Finds approximate solution to equation using Newton-Raphson Method
 * PARAMETERS : Equation
...
function SolveEquation3(leftbound, rightbound, func) { //Uses Newton-Raphson Method
    ...
    for(var i = 0; i <= 1000; i++){
        ...
        try{
            ...
            //Use direct approximation if Newton's Method fails
            if(isNaN(fpx1) ||  Math.abs(fpx1)<= 0.0001){           
...
/*------------------------------------------------------------------------------
 * NAME       : SolveEquationNewtonMethod
 * PURPOSE    : Finds approximate solution to equation using Newton-Raphson Method
 * PARAMETERS : Equation
 * RETURNS    : approximate solution
 *----------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
* NAME       :  EvaluateExpression222
* PURPOSE    :  evaluates an expression in Polish notation
* PARAMETERS :  string expression in Polish notation
* RETURNS    :  numerical value (string)
*----------------------------------------------------------------------------*/         
function EvaluateExpression222(expression) {
...
*----------------------------------------------------------------------------*/     
    function EvaluateExpression(expression) {
        var postFixTT = expression.split(",");//postFix.split(',');
        //while (postFixTT.length > 1) {
        var storage = [];
        for (var i = 0; i < postFixTT.length; i++) {
            var pp = postFixTT[i];
            //alert('pp = ' + pp);
            if(isAlpha(postFixTT[i].toString())){
                postFixTT[i] = (postFixTT[i].toString()).toUpperCase();
            }

            if (IsNumericc(postFixTT[i])) {
                storage.push(postFixTT[i]);
            }
            else if (pp == '^' || pp == '*' || pp == '/' || pp == '+' || pp == '-') {
                var value2 = storage.pop();
                var value1 = storage.pop();
                if (postFixTT[i] == '+') {
                    if (IsNumericc(postFixTT[i - 2]) && IsNumericc(postFixTT[i - 1])) {
                        postFixTT[i] = parseFloat(value1) + parseFloat(value2); //parseFloat(postFixTT[i - 2]) + parseFloat(postFixTT[i - 1]);
                        storage.push(postFixTT[i]);
                    ...
                    if (IsNumericc(postFixTT[i - 2]) && IsNumericc(postFixTT[i - 1])) {
                        postFixTT[i] = parseFloat(value1) - parseFloat(value2);
                        //postFixTT[i] = parseFloat(postFixTT[i - 2]) - parseFloat(postFixTT[i - 1]);
                        storage.push(postFixTT[i]);
                        //postFixTT.splice(i - 2, 2);
                    }
                }
                    ...
                    if (IsNumericc(postFixTT[i - 2]) && IsNumericc(postFixTT[i - 1])) {
                        //postFixTT[i] = Math.pow(parseFloat(postFixTT[i - 2]), parseFloat(postFixTT[i - 1]));
                        postFixTT[i] = PerformPowerOperation(parseFloat(value1), parseFloat(value2));
                        storage.push(postFixTT[i]);
                        //postFixTT.splice(i - 2, 2);
                    }
                }
            }
            else if (IsFunction(postFixTT[i].toString())) {
                //alert('is function...' + postFixTT[i]);
                var FuncName = pp.toString();
                if (FuncName == "RSTRICT" || FuncName == "INQUALITYG" || FuncName == "INQUALITYH"
                    || FuncName == "INQUALITYI" || FuncName == "INQUALITYJ") {
                    //functions with three arguments
                    var value3 = storage.pop();
                    var value2 = storage.pop();
                    var value1 = storage.pop();
                    //postFixTT[i] = FunctionEval(pp.toString(), postFixTT[i - 3].toString(), postFixTT[i - 3].toString(), postFixTT[i - 1].toString(), 0, 0);
                    //postFixTT.splice(i - 3, 3);
                    postFixTT[i] = FunctionEval(pp.toString(), parseFloat(value1), parseFloat(value2), parseFloat(value3), 0, 0);
                    storage.push(postFixTT[i]);
                }
                else if (FuncName == "NROOT" || FuncName == "INQUALITYA" || FuncName == "INQUALITYB" || FuncName == "INQUALITYC" || FuncName == "INQUALITYD" || FuncName == "INQUALITYDD" || FuncName == "INQUALITYF") {
                    //alert('klajsdl;fkjaklsdfjl;aks');
                    //functions with two arguments
                    //alert('storage11 = ' + storage);
                    var value2 = storage.pop();
                    var value1 = storage.pop();
                    //alert('storage = ' + storage);
                    //alert('val1=' + value1 + '.......val2=' + value2);

                    postFixTT[i] = FunctionEval(pp.toString(), parseFloat(value1), parseFloat(value2), 0, 0, 0);
                    storage.push(postFixTT[i]);
                    //postFixTT.splice(i - 2, 2);
                }
                else {
                    ...
                    var value1 = storage.pop();
                    postFixTT[i] = FunctionEval(pp.toString(), parseFloat(value1), 0, 0, 0, 0);
                    storage.push(postFixTT[i]);
                    //postFixTT.splice(i - 1, 1);
                }
            }
            else if (pp == ';') {
                //alert('pp;;;; = ' + pp);
            ...
            //alert('storage = ' + storage + '; length = ' +  storage.length);
        }            
        //alert('storage = ' + storage + '; length = ' +  storage.length);
...
/*------------------------------------------------------------------------------
* NAME       : PerformPowerOperation          
* PURPOSE    : Calculates value of expression
* PARAMETERS : base, exponent
* RETURNS    : value of expression
*----------------------------------------------------------------------------*/
    function PerformPowerOperation(base, exponent) {
        //case 1: base >= 0 or exponent is integer 
        if (base >= 0 || exponent == parseInt(exponent)) {
            return Math.pow(base, exponent);
        }

        //case 2: base < 0  && exponent is rational
        var temp = ToFraction(exponent.toString());
        if (temp.indexOf("/") > -1) {
            var temp2 = temp.split("/");
            if (parseFloat(temp2[1]) % 2 == 1) {//rational exponent has odd denominator and base < 0
                var vem = -1 * Math.pow(-base, 1.0 / parseFloat(temp2[1]));
                return Math.pow(vem, parseFloat(temp2[0]));
            }
            else {//rational exponent has even denominator and base < 0
                 return NaN;
            }
        }
    }
...
/*------------------------------------------------------------------------------
* NAME       :  ProcessPoly
* PURPOSE    :  Simplifies polynomial expression
* PARAMETERS :  string
* RETURNS    :  Simplified polynomial expression
*----------------------------------------------------------------------------*/
 function EvaluateExpression22(expression) {
        //var tokens = expression.split(",");
        ...
        while (postFixTT.length > 1) {
           ...
           //alert('posfFixTT44 = ' + postFixTT);

        }

        //alert('posfFixTT55 = ' + postFixTT);
...
*----------------------------------------------------------------------------*/
 //function is currently not used
    function InfixToPostfix22(expression) {
            ...
            if (isAlpha(currentToken)) {
                ...
                while (operatorStack[operatorStack.length - 1] != '(') {
                    if (operatorStack.length == 0)
                        throw ("Input expression has mismatched parentheses.");
...
*              (i.e. determines order of operation of an operator(+,-,*,/,^))
* PARAMETERS : character
...
/*------------------------------------------------------------------------------
* NAME       : FunctionEval
* PURPOSE    : Evalutes function values
* PARAMETERS : function name; numerical arguments
* RETURNS    : numerical value
*----------------------------------------------------------------------------*/
    function FunctionEval(func, arg1, arg2, arg3, arg4, arg5) {
        ...
        switch (func) {           
            ...
            case "ACOT": return Math.PI/2 - Math.atan(arg1);
            case "ASEC": return Math.acos(1/arg1);
            case "ACSC": return Math.asin(1/arg1);
            ...
            case "TAN":
                var x = arg1 / (0.5 * Math.PI);
                if ((Math.abs(x - parseInt(x)) < 0.0000000000001) && (parseInt(x) % 2 == 1)){
                    return NaN;
                }
                else {
            ...
            case "TANDGG": 
                ...
                if ((Math.abs(x - parseInt(x)) < 0.0000000000001) && (parseInt(x) % 2 == 1)){
                    ...
                    return Math.tan(arg1 * 0.01745329252);
                }                     
            
            case "CSCDGG": 
                var x = arg1 / 180;
                if (Math.abs(x - parseInt(x)) < 0.0000000000001) {
                    return NaN;
                }
                else {
                    return 1.0 / Math.sin(arg1 * 0.01745329252);
                }                        
            ...
            case "SECDGG": 
                ...
                if ((Math.abs(x - parseInt(x)) < 0.0000000000001) && (parseInt(x) % 2 == 1)){
                    ...
                    return 1.0 / Math.cos(arg1 * 0.01745329252);
                }                
            ...
            case "COTDGG": 
                ...
                if (Math.abs(x - parseInt(x)) < 0.0000000000001) {
                    ...
                    return 1.0 / Math.tan(arg1 * 0.01745329252);
                }                     
            ...
            case "SEC": 
                ...
                if ((Math.abs(x - parseInt(x)) < 0.0000000000001) && (parseInt(x) % 2 == 1)){
                    ...
                    return 1.0 / Math.cos(arg1);
                }             
            case "COT":
                var x = arg1 / Math.PI;
                if (Math.abs(x - parseInt(x)) < 0.000000000000001) {
                    return NaN;
                }
                else {
                    return 1.0 / Math.tan(arg1);
                }

            case "SINH": return (Math.pow(Math.E, arg1) - Math.pow(Math.E, -arg1)) / 2.0;
            case "COSH": 
            return (Math.pow(Math.E, arg1) + Math.pow(Math.E, -arg1)) / 2.0;
            case "TANH":
            ...
            case "COTH":  
                ...
                var r = y / x;
                return r;
            case "INVSINH": return Math.log(arg1 + Math.sqrt(arg1*arg1+1));
            case "INVCOSH": return Math.log(arg1 + Math.sqrt(arg1*arg1-1));
            case "INVTANH": return (0.5)*Math.log(  (1+arg1)/(1-arg1)    );
            case "INVCOTH": return (0.5)*Math.log((arg1+1)/(arg1-1));
            case "INVCSCH": return Math.log(1/arg1 + Math.sqrt(arg1*arg1+1)/Math.abs(arg1));
            case "INVSECH": return Math.log(  (1 + Math.sqrt(1-arg1*arg1))/arg1);
            case "NROOT":
                if(arg2 < 0){
...
*----------------------------------------------------------------------------*/
    ...
    *----------------------------------------------------------------------------*/
    function IsFunction(func) {
        ...
        var ListFuncOps = ["AVG", "ABS", "ACOS", "ASC", "ASIN", "ATAN", "ACOT", "ASEC", "ACSC", "CDATE", "CHR", "COS", "FAC",
                        ...
                        "RSTRICT", "SINH", "COSH", "TANH", "CSCH", "SECH", "COTH", "LCM", "GCF", "INVSINH", "INVCOSH", "INVTANH", "INVCSCH", "INVSECH", "INVCOTH",
...
/*------------------------------------------------------------------------------
* NAME       : Normalpdf
* PURPOSE    : Finds y-value of normal distribution pdf
* PARAMETERS : x-score, mean, standard deviation,
* RETURNS    : y-value of normal distribution pdf
*----------------------------------------------------------------------------*/ 
...
/*------------------------------------------------------------------------------
* NAME       : Normalcdf
* PURPOSE    : Finds y-value of normal distribution cdf
* PARAMETERS : x-score, mean, standard deviation,
* RETURNS    : y-value of normal distribution cdf
*----------------------------------------------------------------------------*/ 
...
/*------------------------------------------------------------------------------
* NAME       : tDistPDF
* PURPOSE    : Finds y-value of t-distribution pdf
* PARAMETERS : degrees of freedom, t-score
* RETURNS    : y-value of t-distribution pdf
*----------------------------------------------------------------------------*/ 
...
/*------------------------------------------------------------------------------
* NAME       : tDistCDF
* PURPOSE    : Finds y-value of t-distribution cdf
* PARAMETERS : degrees of freedom, t-score
* RETURNS    : y-value of t-distribution cdf
*----------------------------------------------------------------------------*/ 
...
/*------------------------------------------------------------------------------
* NAME       :  factorial100
* PURPOSE    :  Find n!  (n is between 1 and 100)
* PARAMETERS :  integer
* RETURNS    :  value of n!
*----------------------------------------------------------------------------*/
...
* PURPOSE    :  Turns on selected calculator feature
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/    
    function CalculatorFeature() {
        var val = document.getElementById('mydropdownCalculatorFeature').value;
        if (val == 1) {
            document.getElementById('ReflectionCheckbox').checked = true;
            ReflectionCheckboxChecked();
        }
        else if (val == 2) {
            document.getElementById('RotationCheckbox').checked = true;
            TurnOnRotationFeature();
        }
        else if (val == 3) {
            document.getElementById('XYInterceptCheckbox').checked = true;
            XYInterceptCheckboxClick();
        }
        else if (val == 4) {
            document.getElementById('IntersectionCheckbox').checked = true;
            IntersectionCheckboxClick();
        }
        else if (val == 5) {
            document.getElementById('ExtremumCheckbox').checked = true;
            ExtremumCheckboxClick();
        }
        else if (val == 6) {
            ...
            document.getElementById('DrawingLineCheckbox11').checked = true;
            DrawingLine11();
        }
        else if (val == 8) {
            document.getElementById('DrawingLineCheckbox22').checked = true;
            DrawingLine22();
        }
        else if (val == 9) {
            document.getElementById('DrawingLineCheckbox33').checked = true;
            DrawingLine33();
        }
        else if (val == 10) {
            document.getElementById('DrawingLineCheckbox55').checked = true;
            DrawingLine55();
        }
        else if (val == 11) {
            document.getElementById('DrawingLineCheckbox88').checked = true;
            DrawingLine88();
        }
        else if (val == 12) {
            document.getElementById('DrawingLineCheckbox99').checked = true;
            DrawingLine99();
        }
        else if (val == 13) {
            document.getElementById('DrawingLineCheckbox66').checked = true;
            DrawingLine66();
        }
        else if (val == 14) {
            document.getElementById('DrawingLineCheckbox77').checked = true;
            DrawingLine77();
        }
        else if (val == 15) {
            document.getElementById('DrawingLineCheckbox1010').checked = true;
            DrawingLine1010();
        }
        else if (val == 16) {
            document.getElementById('DrawingLineCheckbox1111').checked = true;
            DrawingLine1111();
        }
        else if (val == 17) {
            TracingCartesian('1');
            ...
            document.getElementById('DrawingLineCheckbox44').checked = true;
            DrawingLine44();
        }
        else if (val == 20) {
            document.getElementById('graphOptions').style.display = 'inline-block';			
        }
    ...
    function CalculatorFeature22(val) {
        //var val = document.getElementById('mydropdownCalculatorFeature').value;
        if (val == 1) {
            document.getElementById('ReflectionCheckbox').checked = true;
            ReflectionCheckboxChecked();
...
/*------------------------------------------------------------------------------
* NAME       :  TracingCartesian
* PURPOSE    :  Displays manula or automatic tracing panel
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/    
    function TracingCartesian(casenum){
        if(casenum == 1){
            document.getElementById('tracingpanel').style.display = 'inline-block';
            document.getElementById('tracingpanel').style.width = '610px';
            document.getElementById('tracingpanel').style.height = '420px';
        }            
        else {
            ...
            document.getElementById('TracePolarCheckbox').checked = false;
            document.getElementById('TraceParametricCheckbox').checked = false;

            document.getElementById('tracingmethod1').checked = true;
            document.getElementById('tracingpanel').style.display = 'none';
        }
    }

/*------------------------------------------------------------------------------
* NAME       :  ClearShading22
* PURPOSE    :  Displays or hides shading panel
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/    
    function ClearShading22() {
        if (document.getElementById('ShadingCheckbox').checked) {
            window.scroll(0, 0);
            document.getElementById('Shadingpanel').style.display = 'inline-block';
            document.getElementById('Shadingpanel').style.width = '600px';
            document.getElementById('Shadingpanel').style.height = '350px';
            ShadeFeatureOnOff('2');
        }
        else {
            document.getElementById('RightSideForm').style.backgroundColor = 'white';
            ClearShading();
        }
    }

/*------------------------------------------------------------------------------
* NAME       :  ClearShading3344
* PURPOSE    :  Displays or hides shading panel
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/    
    function ClearShading3344() {
        document.getElementById('ShadingCheckbox').checked = false;
        document.getElementById('Shadingpanel').style.display = 'none';
        ClearShading22();
        ShadeFeatureOnOff('1');
    }
...
/*------------------------------------------------------------------------------
* NAME       :  ReflectionCheckboxChecked
* PURPOSE    :  Displays or hides reflection panel
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function ReflectionCheckboxChecked() {
        if (document.getElementById('ReflectionCheckbox').checked) {
            window.scroll(0, 0);
            document.getElementById('Reflectionpanel').style.display = 'inline-block';
            document.getElementById('Reflectionpanel').style.width = '600px';
            document.getElementById('Reflectionpanel').style.height = '250px';
            GraphingCalculator();
        }
        else {
            Turnoffreflectionfeature();
        }
...
/*------------------------------------------------------------------------------
* NAME       :  Turnoffreflectionfeature
* PURPOSE    :  Displays or hides reflection panel
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function Turnoffreflectionfeature() {
        document.getElementById('ReflectionCheckbox').checked = false;
        document.getElementById('Reflectionpanel').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOnRotationFeature
* PURPOSE    :  Displays or hides rotation panel
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOnRotationFeature() {
        if (document.getElementById('RotationCheckbox').checked) {
            window.scroll(0, 0);
            document.getElementById('Rotationpanel').style.display = 'inline-block';
            document.getElementById('Rotationpanel').style.width = '600px';
            document.getElementById('Rotationpanel').style.height = '250px';
            GraphingCalculator();
        }
        else {
            TurnOffRotationFeature();
        }
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffRotationFeature
* PURPOSE    :  Displays or hides rotation panel
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffRotationFeature() {
        document.getElementById('RotationCheckbox').checked = false;
        document.getElementById('Rotationpanel').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :   XYInterceptCheckboxClick
* PURPOSE    :   Turns on/off "find x- or y-intercept" feature
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function XYInterceptCheckboxClick() {
        if (document.getElementById('XYInterceptCheckbox').checked) {
            window.scroll(0, 0);
            document.getElementById('interceptpanel').style.display = 'inline-block';
            document.getElementById('interceptpanel').style.width = '600px';
            document.getElementById('interceptpanel').style.height = '300px';
            document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
            TurnOffXYInterceptFeature();
        }
    }

/*------------------------------------------------------------------------------
* NAME       :   TurnOffXYInterceptFeature
* PURPOSE    :   Turns on/off "find x- or y-intercept" feature
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffXYInterceptFeature() {
        document.getElementById('XYInterceptCheckbox').checked = false;
        document.getElementById('interceptpanel').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :   IntersectionCheckboxClick
* PURPOSE    :   Turns on/off "find intersection" feature
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/     
    function IntersectionCheckboxClick() {
        if (document.getElementById('IntersectionCheckbox').checked) {
            window.scroll(0, 0);
            document.getElementById('intersectionpanel').style.display = 'inline-block';
            document.getElementById('intersectionpanel').style.width = '600px';
            document.getElementById('intersectionpanel').style.height = '350px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
            TurnOffIntersectionFeature();
        }
    }
...
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffIntersectionFeature() {
        document.getElementById('IntersectionCheckbox').checked = false;
        document.getElementById('intersectionpanel').style.display = 'none';
    }

 /*------------------------------------------------------------------------------
* NAME       :   ExtremumCheckboxClick
* PURPOSE    :   Turns on/off "Find extremum" feature
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function ExtremumCheckboxClick() {
        if (document.getElementById('ExtremumCheckbox').checked) {
            window.scroll(0, 0);
            document.getElementById('Extremumpanel').style.display = 'inline-block';
            document.getElementById('Extremumpanel').style.width = '600px';
            document.getElementById('Extremumpanel').style.height = '600px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
            TurnOffExtremumFeature();
        }
    }

 /*------------------------------------------------------------------------------
* NAME       :   TurnOffExtremumFeature
* PURPOSE    :   Turns on/off "Find extremum" feature
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffExtremumFeature() {
        document.getElementById('ExtremumCheckbox').checked = false;
        document.getElementById('Extremumpanel').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  DrawingLine11 (Drawing Line With Two Given Points) 
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine11() {
        if (document.getElementById('DrawingLineCheckbox11').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel1').style.display = 'inline-block';
            document.getElementById('Linepanel1').style.width = '600px';
            document.getElementById('Linepanel1').style.height = '300px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel1  (Drawing Line With Two Given Points) 
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel1() {
        document.getElementById('DrawingLineCheckbox11').checked = false;
        document.getElementById('Linepanel1').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel22 (Drawing Line With Given Slope and One Point)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine22() {
        if (document.getElementById('DrawingLineCheckbox22').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel2').style.display = 'inline-block';
            document.getElementById('Linepanel2').style.width = '600px';
            document.getElementById('Linepanel2').style.height = '300px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel2 (Drawing Line With Given Slope and One Point)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel2() {
        document.getElementById('DrawingLineCheckbox22').checked = false;
        document.getElementById('Linepanel2').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel33 (Drawing Parabola Through Vertex and One Point)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine33() {
        if (document.getElementById('DrawingLineCheckbox33').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel3').style.display = 'inline-block';
            document.getElementById('Linepanel3').style.width = '600px';
            document.getElementById('Linepanel3').style.height = '300px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel3 (Drawing Parabola Through Vertex and One Point)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel3() {
        document.getElementById('DrawingLineCheckbox33').checked = false;
        document.getElementById('Linepanel3').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  DrawingLine44 Drawing Parabola Through Three Points)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine44() {
        if (document.getElementById('DrawingLineCheckbox44').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel4').style.display = 'inline-block';
            document.getElementById('Linepanel4').style.width = '600px';
            document.getElementById('Linepanel4').style.height = '300px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel4  
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel4() {
        document.getElementById('DrawingLineCheckbox44').checked = false;
        document.getElementById('Linepanel4').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel55 (Drawing Circle)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine55() {
        if (document.getElementById('DrawingLineCheckbox55').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel5').style.display = 'inline-block';
            document.getElementById('Linepanel5').style.width = '600px';
            document.getElementById('Linepanel5').style.height = '300px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel5 (Drawing Circle)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel5() {
        document.getElementById('DrawingLineCheckbox55').checked = false;
        document.getElementById('Linepanel5').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel66 (Drawing Two Parralel Lines)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine66() {
        if (document.getElementById('DrawingLineCheckbox66').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel6').style.display = 'inline-block';
            document.getElementById('Linepanel6').style.width = '600px';
            document.getElementById('Linepanel6').style.height = '300px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel6 (Drawing Two Parralel Lines)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel6() {
        document.getElementById('DrawingLineCheckbox66').checked = false;
        document.getElementById('Linepanel6').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel77 (Drawing Two Perpendicular Lines)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine77() {
        if (document.getElementById('DrawingLineCheckbox77').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel7').style.display = 'inline-block';
            document.getElementById('Linepanel7').style.width = '600px';
            document.getElementById('Linepanel7').style.height = '300px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel7 (Drawing Two Perpendicular Lines)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel7() {
        document.getElementById('DrawingLineCheckbox77').checked = false;
        document.getElementById('Linepanel7').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel88 (Drawing Ellipse of Equation in Standard Form)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine88() {
        if (document.getElementById('DrawingLineCheckbox88').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel8').style.display = 'inline-block';
            document.getElementById('Linepanel8').style.width = '600px';
            document.getElementById('Linepanel8').style.height = '300px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel8 (Drawing Ellipse of Equation in Standard Form)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel8() {
        document.getElementById('DrawingLineCheckbox88').checked = false;
        document.getElementById('Linepanel8').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel99 (Drawing Hyperbola of Equation in Standard Form)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine99() {
        if (document.getElementById('DrawingLineCheckbox99').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel9').style.display = 'inline-block';
            document.getElementById('Linepanel9').style.width = '600px';
            document.getElementById('Linepanel9').style.height = '350px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel9 (Drawing Hyperbola of Equation in Standard Form)
* PURPOSE    :  Turns on/off "Draws line" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel9() {
        document.getElementById('DrawingLineCheckbox99').checked = false;
        document.getElementById('Linepanel9').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel1010 (Testing Point for Inequality and Equation)
* PURPOSE    :  Turns on/off "Testing Point for Inequality and Equation" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine1010() {
        if (document.getElementById('DrawingLineCheckbox1010').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel10').style.display = 'inline-block';
            document.getElementById('Linepanel10').style.width = '600px';
            document.getElementById('Linepanel10').style.height = '550px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel10 (Testing Point for Inequality and Equation)
* PURPOSE    :  Turns on/off "Testing Point for Inequality and Equation" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel10() {
        document.getElementById('DrawingLineCheckbox1010').checked = false;
        document.getElementById('Linepanel10').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel1111 (Draw Tangent Line(s) for function defined implicitly or explicitly)
* PURPOSE    :  Turns on/off "Draw Tangent Line(s) for function defined implicitly or explicitly" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function DrawingLine1111() {
        if (document.getElementById('DrawingLineCheckbox1111').checked) {
            window.scroll(0, 0);
            document.getElementById('Linepanel11').style.display = 'inline-block';
            document.getElementById('Linepanel11').style.width = '600px';
            document.getElementById('Linepanel11').style.height = '550px';
            //document.getElementById('LocationXIntercept').value = '';
            GraphingCalculator();
        }
        else {
...
/*------------------------------------------------------------------------------
* NAME       :  TurnOffLinepanel11 (Draw Tangent Line(s) for function defined implicitly or explicitly)
* PURPOSE    :  Turns on/off "Draw Tangent Line(s) for function defined implicitly or explicitly" feature 
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TurnOffLinepanel11() {
        document.getElementById('DrawingLineCheckbox1111').checked = false;
        document.getElementById('Linepanel11').style.display = 'none';
    }

/*------------------------------------------------------------------------------
* NAME       :   graphcolor
* PURPOSE    :   Sets color of graph
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function graphcolor(i) {
        if (i == 1) { buttoncolor = "blue"; }
        else if (i == 2) { buttoncolor = "red"; }
        else if (i == 3) { buttoncolor = "green"; }
        else if (i == 4) { buttoncolor = "orange"; }
        else if (i == 5) { buttoncolor = "lime"; }
        else if (i == 6) { buttoncolor = "cyan"; }
        else if (i == 7) { buttoncolor = "magenta"; }
        else if (i == 8) { buttoncolor = "silver"; }
        else if (i == 9) { buttoncolor = "gray"; }
        else if (i == 10) { buttoncolor = "maroon"; }
        else if (i == 11) { buttoncolor = "olive"; }
        else if (i == 12) { buttoncolor = "purple"; }
        else if (i == 13) { buttoncolor = "teal"; }
        else if (i == 14) { buttoncolor = "salmon"; }
        else if (i == 15) { buttoncolor = "gold"; }
        else if (i == 16) { buttoncolor = "khaki"; }
        else if (i == 17) { buttoncolor = "indigo"; }
        else if (i == 18) { buttoncolor = "#1E90FF"; }
        else if (i == 19) { buttoncolor = "#483D8B"; }
        else if (i == 20) { buttoncolor = "#B8860B"; }

        return buttoncolor;
    }

/*------------------------------------------------------------------------------
* NAME       :   SelectCartesianGraphTracing
* PURPOSE    :   Selects grahp to be traced
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function SelectCartesianGraphTracing(){
        var val = document.getElementById('mydropdowntracing').value;
        document.getElementById("tracegraph"+val).checked = true;
        document.getElementById('mydropdowntracing').style.color = graphcolor(val);
        TraceCartesianGraphID(); 
    }
...
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function SelectCartesianGraphTracing2(num){
        var val = document.getElementById('mydropdowndifference'+num).value;
        document.getElementById('mydropdowndifference'+num).style.color = graphcolor(val);
    }


/*------------------------------------------------------------------------------
* NAME       :   dropdownboxpolar
* PURPOSE    :   Sets color for polar graph
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function dropdownboxpolar(){
        var val = document.getElementById('mydropdownpolar').value;
        //document.getElementById("tracegraph"+val).checked = true;
        document.getElementById('mydropdownpolar').style.color = graphcolor(val);
    }
    
/*------------------------------------------------------------------------------
* NAME       :   selectacolor11bb
* PURPOSE    :   Displays selected color
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function selectacolor11bb() {
        var val = document.getElementById('mydropdownShadingColor').value;
        if (val == 0) {
            document.getElementById('mydropdownShadingColor').style.color = '#eae6e0';
            return;
        }

        document.getElementById('mydropdownShadingColor').style.color = graphcolor(val);

    }

/*------------------------------------------------------------------------------
* NAME       :   Selectanequation12
* PURPOSE    :   Displays selected color
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function Selectanequation12() {
        var val = document.getElementById('mydropdownreflection').value;
        if (val == 0) {
            document.getElementById('mydropdownreflection').style.color = '#eae6e0';
            return;
        }

        document.getElementById('mydropdownreflection').style.color = graphcolor(val);

    }

/*------------------------------------------------------------------------------
* NAME       :   Selectanequation14
* PURPOSE    :   Displays selected color
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function Selectanequation14() {
        var val = document.getElementById('mydropdownreflection22').value;
        if (val == 0) {
            document.getElementById('mydropdownreflection22').style.color = '#eae6e0';
            return;
        }

        document.getElementById('mydropdownreflection22').style.color = graphcolor(val);

    }

/*------------------------------------------------------------------------------
* NAME       :   Selectanequation77
* PURPOSE    :   Displays selected color
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function Selectanequation77() {
        var val = document.getElementById('mydropdownreflection77').value;
        if (val == 0) {
            document.getElementById('mydropdownreflection77').style.color = '#eae6e0';
            GraphingCalculator();
            return;
        }

        document.getElementById('mydropdownreflection77').style.color = graphcolor(val);
        GraphingCalculator();
    }

/*------------------------------------------------------------------------------
* NAME       :   Selectanequation24
* PURPOSE    :   Displays selected color
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function Selectanequation24() {

        var val = document.getElementById('mydropdownRotation').value;
        if (val == 0) {
            document.getElementById('mydropdownRotation').style.color = '#eae6e0';
            return;
        }

        document.getElementById('mydropdownRotation').style.color = graphcolor(val);

    }

/*------------------------------------------------------------------------------
* NAME       :   Selectanequation34
* PURPOSE    :   Displays selected color
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function Selectanequation34() {
        var val = document.getElementById('mydropdownXYIntercept').value;
        if (val == 0) {
            document.getElementById('mydropdownXYIntercept').style.color = '#eae6e0';
            return;
        }

        document.getElementById('mydropdownXYIntercept').style.color = graphcolor(val);
...
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function intersectEqu1() {
        var val = document.getElementById('mydropdownIntersectionAA').value;
        if (val == 0) {
            document.getElementById('mydropdownIntersectionAA').style.color = '#eae6e0';
            return;
        }

        document.getElementById('mydropdownIntersectionAA').style.color = graphcolor(val);
...
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function intersectEqu2() {
        var val = document.getElementById('mydropdownIntersectionBB').value;
        if (val == 0) {
            document.getElementById('mydropdownIntersectionBB').style.color = '#eae6e0';
            return;
        }

        document.getElementById('mydropdownIntersectionBB').style.color = graphcolor(val);
    }

/*------------------------------------------------------------------------------
* NAME       :   TableCartesianOpenClose
* PURPOSE    :   Displays or hides table of values for Cartesian graph
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function TableCartesianOpenClose(val) {
        if (val == 1) {
            document.getElementById('tablenew').style.display = 'inline-block';
        }
        else {
            document.getElementById('tablenew').style.display = 'none';
        }

    }

/*------------------------------------------------------------------------------
* NAME       :   PolarEquationTableCheckBoxNewBB
* PURPOSE    :   Turns on/off checkbox for polar equation
* PARAMETERS :   
* RETURNS    :   
*----------------------------------------------------------------------------*/ 
    function PolarEquationTableCheckBoxNewBB() {
        if (document.getElementById('PolarEquationTableCheckBoxNew').checked) {
            document.getElementById('PolarEquationTableCheckBox').checked = true;
        }
        else {
            document.getElementById('PolarEquationTableCheckBox').checked = false;
        }
