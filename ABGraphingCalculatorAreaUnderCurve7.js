     ...
     function draw() { //draw()22
        ...
        for (var i = 1; i <= 20; i++) {
            ...
            if (ee1 != "" && ee2 && parseInt(valuedropbox) <= 4 ) {
                ...
                if(i==111){//case1
                    for(var jj=0; jj<dataCX.length-100; jj++){
                        ctx.globalAlpha = 0.2;
                        var xtt = dataCX[jj];
                        var ytt = dataCY[jj];
                        //alert('x='+xtt+'....y='+ytt);
                ...
                if(i==1){//case1
                    var bb = FindValue(document.getElementById('limitvalue2').value);
                    var ellipseFactor = document.getElementById('ellipseFactor').value;
                    var hh = parseFloat(bb) - Math.sqrt(ellipseFactor);
                    for (var jj = 0; jj < dataCX.length; jj++) {
                        ctx.globalAlpha = 0.2;
                        var xtt = dataCX[jj]; //data from top half of function
                        var ytt = dataCY[jj];
                        var dataX = [xtt, xtt];
                        var dataY = [ytt, 0];

                        if (parseFloat(xtt) <= hh) {
                            //funGraph(ctx, "LIMIT", 'red', 3, dataX, dataY, xRminn, xRmaxx, yRminn, yRmaxx, x0, y0, dxx, dyy, dashFlag, ShadeFlag, '0.3');
                        }
                    }

                    var dataE = document.getElementById('storge22').value; //theta
                    if (dataE.trim() != '') {
                        var dataF = dataE.split(";;");
                        for (var jj = 0; jj < dataF.length; jj++) {
                            var dataG = dataF[jj].split(",");
                            var dataX = [dataG[0], dataG[0]];
                            var dataY = [dataG[1], dataG[2]];
                            //funGraph(ctx, "LIMIT", 'red', 3, dataX, dataY, xRminn, xRmaxx, yRminn, yRmaxx, x0, y0, dxx, dyy, dashFlag, ShadeFlag, '0.3');
                ...
                if(i==20){
                    ...
                    var dataE = document.getElementById('storge22').value; //
                    if (dataE.trim() != '') {
                        var dataF = dataE.split(";;");
                        for (var jj = 0; jj < dataF.length; jj++) {
                            var dataG = dataF[jj].split(",");
                            var y1 = -1 * parseFloat(dataG[1]);
                            var y2 = -1 * parseFloat(dataG[2]);
                            var dataX = [dataG[0], dataG[0]];
                            var dataY = [y1, y2];
                            //funGraph(ctx, "LIMIT", 'red', 3, dataX, dataY, xRminn, xRmaxx, yRminn, yRmaxx, x0, y0, dxx, dyy, dashFlag, ShadeFlag, '0.3');
                        }
         ...
         //curves
         //ctx.beginPath();
         //var xx1 = x0 + parseFloat(5) * dxx;
         //var yy1 = y0 - parseFloat(5) * dyy;
         //var xx2 = x0 + parseFloat(3) * dxx;
         //var yy2 = y0 - parseFloat(4) * dyy;
         //var xx3 = x0 + parseFloat(3) * dxx;
         //var yy3 = y0 - parseFloat(-4) * dyy;
         //var xx4 = x0 + parseFloat(5) * dxx;
         //var yy4 = y0 - parseFloat(-5) * dyy;
         //ctx.moveTo(xx1, yy1);
         //ctx.quadraticCurveTo(xx2, yy2, xx3, yy3);
         ...
         var dataE = document.getElementById('storge22').value; //
         if (dataE.trim() != '') {
             var datatheta = document.getElementById('storge22').value; //theta
             var dataradius = document.getElementById('storge33').value; //radius

             //alert('data22=' + data22)

             var data22 = datatheta.split(";");
             var data33 = dataradius.split(";");
             var nn = 0;  //document.getElementById('limitvalue3').value;
             if (nn <= 10) {
                 for (var i = 1; i <= nn; i++) {
                     var X1 = document.getElementById("Point" + i + "X").value;
                     var Y1 = document.getElementById("Point" + i + "Y").value;
                     if (X1.trim() != '' && Y1.trim() != '') {
                         var r1 = Math.sqrt(parseFloat(X1) * parseFloat(X1) + parseFloat(Y1) * parseFloat(Y1));
                         var X2 = document.getElementById("Point" + (i + 1) + "X").value;
                         var Y2 = document.getElementById("Point" + (i + 1) + "Y").value;
                         var r2 = Math.sqrt(parseFloat(X2) * parseFloat(X2) + parseFloat(Y2) * parseFloat(Y2));
                         var Inputradius = Math.max(r1, r2);

                         var Inputstartangle = Math.atan(Y1 / X1);
                         var Inputendangle = Math.atan(Y2 / X2);
                         var InputcenterX = 0;
                         var InputcenterY = 0;
                         var radius = x0 + parseFloat(Inputradius) * dxx-270;
                         var centerX = x0 + parseFloat(InputcenterX) * dxx;
                         var centerY = x0 + parseFloat(InputcenterY) * dxx;
                         var startangle = 2 * Math.PI - Inputendangle;
                         var endangle = 2 * Math.PI - Inputstartangle;
                         ctx.beginPath();
                         ctx.arc(centerX, centerX, radius, startangle, endangle); //(center-x, center-y, radius, start angle, end angle)
                         ctx.stroke();
                     }
         ...
         if (document.getElementById('turnonoffsectors').checked) {
             var data44 = document.getElementById('storge44').value;
             if (data44.indexOf(';;') > -1) {
                 data44A = data44.split(';;');
                 //alert('data44A...' + data44A.length);
                 for (var i = 0; i < data44A.length; i++) {
                     data44B = data44A[i].split(';');
                     //data44C = data44A[i+1].split(';');
                     var dataXpolar = [data44B[0], 0];
                     var dataYpolar = [data44B[1], 0];
                     var color = 'gray';
                     if (i == 0) {
                         color = 'green';
                     }
                     if (i == data44A.length - 1) {
                         color = 'red';
                     }

                     funGraph(ctx, "LIMIT", color, 3, dataXpolar, dataYpolar, xRminn, xRmaxx, yRminn, yRmaxx, x0, y0, dxx, dyy, dashFlag, ShadeFlag, '0.3');
         ...
         if (document.getElementById('turnonoffsectors').checked) {
             ...
             if (nn <= 10) {
                 for (j = 1; j <= nn; j++) {
                     var arc = document.getElementById('acrdata' + j).value;
                     if (arc.indexOf(';;') > -1) {
                         //alert('arc1 = ' + arc1);
                         var arcArray = arc.split(';;');
                         for (var i = 0; i < arcArray.length - 1; i++) {
                             var gg = arcArray[i].split(';');
                             var hh = arcArray[i + 1].split(';');
                             var dataXpolar = [gg[0], hh[0]];
                             var dataYpolar = [gg[1], hh[1]];
                             funGraph(ctx, "LIMIT", 'orange', 4, dataXpolar, dataYpolar, xRminn, xRmaxx, yRminn, yRmaxx, x0, y0, dxx, dyy, dashFlag, ShadeFlag, '1');
...
function insertAtCaret(element, text, val1, val2, val3, val4) {
    ...
    function Example33(){
        ...
        document.getElementById('LimitFunction').value  = '3Sin(t) + 5';
        document.getElementById("TextBoxForCartesianGraph1").value = '';
        document.getElementById('limitvalue1').value = 'π/6';
        document.getElementById('limitvalue2').value = 'π/2';
        document.getElementById('limitvalue3').value = '5';
        FindArea();
    }
    ...
    function FindArea() {//draw()22
        ...
        if (InputExpression == "") {
            alert('Please input function r(t).');
            return;
        }
        ...
        bb = FindValue(bb);
        nn = FindValue(nn);
        //document.getElementById('TextBoxForCartesianGraph3').value = (Math.tan(aa))+'x' + '{x ≥ 0}';
        //document.getElementById('TextBoxForCartesianGraph2').value = (Math.tan(bb))+'x' + '{x ≥ 0}';
        
        
        var thetasymbol = String.fromCharCode(952);   //theta   
        var InputExpression3 = replaceAll(InputExpression, 't', thetasymbol);

        document.getElementById("PolarEquationsOnOff").checked = true;
        document.getElementById('checkboxPolarEquation1').checked = true;
        document.getElementById("PolarEquation1").value = InputExpression3;
        document.getElementById("PolarTmin").value = aa;
        document.getElementById("PolarTmax").value = bb;
        document.getElementById("PolarTstepsize").value = 0.1;


        var thetastepsize = (parseFloat(bb) - parseFloat(aa)) / nn;
        //alert('thetastepsize=' + thetastepsize);
        var text = '';
        ...
        document.getElementById('TableDataLimitY1').innerHTML = 'Table of Areas of Sectors:<br>';
        var text = '';
        var SumArea = 0;
        var text33 = '';
        var nn1 = 1;
        ...
        var text22 = '';var text44 = '';
        var text55 = ''; 
        var text66 = '';
        var text11 = '';
        document.getElementById('TableDataLimitY1CBA').innerHTML = '';
        for (var i = 1; i <= nn; i++){
            var leftval = aa + (i - 1) * thetastepsize;
            var rightval = aa + i * thetastepsize;
            var leftval22 = Math.round(leftval*1000000000000)/1000000000000;
            if (i <= 7) {
                text33 = text33 + '&theta;<sub><sub>' + (i - 1) + '</sub></sub> = ' + leftval22 + ' , ';
            }

            text55 = text55 + ';' + rightval
            ...
            var temp1 = replaceAll(InputExpression, "t", "(" + leftval.toString() + ")");
            var fleft = FindValue(temp1);
            
            var temp1 = replaceAll(InputExpression, "t", "(" + rightval.toString() + ")");
            var fright = FindValue(temp1);

            if (i == 1) {
                text55 = text55 + ';' + leftval;
                text66 = text66 + ';' + fleft;                
            }

            text55 = text55 + ';' + rightval;  //theta
            text66 = text66 + ';' + fright; //radius
           ...
           //case 1: Estimate area with inscribed rectangles
            if (casenum == 5) {
                if(i==1){
                    tt = leftval;
                    var temp1 = replaceAll(InputExpression, "t", "(" + tt + ")");
                    var radius = FindValue(temp1);
                    if (nn <= 10) {
                        document.getElementById('Point' + i + 'X').value = radius * Math.cos(tt);
                        document.getElementById('Point' + i + 'Y').value = radius * Math.sin(tt);
                    }
                    text11 = text11 + ';;' + (radius*Math.cos(tt)) + ';' + (radius*Math.sin(tt));
                 }        
                ...
                var temp1 = replaceAll(InputExpression, "t", "(" + leftval + ")");
                var radius22 = FindValue(temp1);


                //generates data for arc
                var arcdata = (radius22*Math.cos(leftval)) + ';' + (radius22*Math.sin(leftval));
                var kk = 0;
                if(nn <=10){
                    var steptt = (rightval - leftval) / 100;
                    var ss = leftval;
                    while (kk <= 100) {
                        arcdata = arcdata + ';;' + (radius*Math.cos(ss)) + ';' + (radius*Math.sin(ss));
                        ss = ss + steptt;
                        kk = kk + 1;
                        if (kk > 100) { break }
                    }
                    
                    document.getElementById('acrdata'+i).value = arcdata;//.substring(2);
                }
                ...
                data3 = data3 + ';' + radius;
                data4 = data4 + ';' + radius;
                var Area = 0.5* deltaX*radius*radius;
                SumArea += Area;
                var text = text + '<tr>'
                   + '<td>'
                   + 'Sector ' + i
                   ...
                   + '</td>'
                   + '<td>'
                   + 'central angle = ' + delta + '&theta;<sub>' + i + '</sub> = ' + deltaX
                   + '</td>'
                   + '<td>'
                   + 'Area   = 0.5(' + radius+ ')<sup>2</sup>(' + deltaX + ')=' + Math.round(Area*1000000000000)/1000000000000
                   + '</td>'
                   + '</tr>';

                if(i<=10){
                    var text22  =  'For Sector #' + i + ':<br>'
                    + '&theta;<sub><sub>' + (i-1) + '</sub></sub>  = ' + leftval + '; &theta;<sub><sub>' + (i) + '</sub></sub> = ' + rightval + '<br>'
                    + '&Delta;&theta;<sub><sub>' + (i) + '</sub></sub> = central angle of sector #'+ i +' = &theta;<sub><sub>' + (i) + '</sub></sub>  - ' + '&theta;<sub><sub>' + (i-1) + '</sub></sub> = ' + deltaX  + '<br>'
                    + 'r(&theta;<sub><sub>'+i+'</sub></sub>) = radius of sector #'+i+' = r(&theta;<sub><sub>' + i + '</sub></sub>) = r(' + tt + ') = ' + radius + '<br>'
                    + 'A<sub><sub>'+i+'</sub></sub> = Area of sector #'+i+' = 0.5&bullet;[r(&theta;<sub><sub>'+i+'</sub></sub>)]<sup>2</sup>&bullet;&Delta;&theta;<sub><sub>'+i+'</sub></sub>  = ' + Math.round(Area * 1000000000000) / 1000000000000 + '<br><br><br>'
 
                    document.getElementById('TableDataLimitY1CBA').innerHTML += text22;

                }

                if (i <= 3) {
                    var text44  = text44 + ' + 0.5[r(&theta;<sub><sub>' + i + '</sub></sub>)]<sup>2</sup>&bullet;' + delta + '&theta;<sub>' + i + '</sub>';
                }
                ...
                if (nn <= 10) {
                    document.getElementById('Point' + (i+1) + 'X').value = radius*Math.cos(tt);
                    document.getElementById('Point' + (i+1) + 'Y').value = radius*Math.sin(tt);
                    //document.getElementById('TextBoxForCartesianGraph' + (i+3)).value = (Math.tan(tt))+'x' + '{x ≥ 0}';
                }

                if (nn > 10 && i<15) {
                    document.getElementById('TextBoxForCartesianGraph' + (i+3)).value = '';
                }
         ...
         var text11 = text11.substring(2);
         if (nn > 10) {
             for (var i = 1; i <= 10; i++) {
                 document.getElementById('acrdata' + i).value = '';
             }
         }

        document.getElementById('TableDataLimitY1C').innerHTML = '&Delta;&theta; = central angle of <i>i</i>th sector = (&beta; - &alpha;)/n = ' + deltaX + '<br><br>';
        text33 = text33 + '......';
        document.getElementById('TableDataLimitY1C').innerHTML += text33;

        var text44 = text44.substring(2);
        var text44 = 'Sum of Areas of Sectors<br> = Approximate area of polar region <br>= ' + text44 ;
        document.getElementById('TableDataLimitY1CBA').innerHTML += text44;
        ...
        var text44  = '0.5[r(&theta;<sub><sub>i</sub></sub>)]<sup>2</sup>&bullet;' + delta + '&theta;<sub>i</sub>';

 
        document.getElementById('SumAreas11').value = nn;
        document.getElementById('SumAreas44').innerHTML = 'i = 1';
        document.getElementById('SumAreas33b').innerHTML = text44 + ' = ' + Math.round(SumArea*1000000000000)/1000000000000;
        ...
        document.getElementById('TableDataLimitY1CBA').innerHTML += '<br><br>Sum of Areas of Sectors = ' + Math.round(SumArea*1000000000000)/1000000000000;


       //document.getElementById('TableDataLimitY1CBA').innerHTML += '<br><br><br><img src="areaundercurve7.png" alt="blank"  />';



        document.getElementById('TableDataLimitY1').innerHTML +='<table border="1" style="white-space: nowrap">' + text + '<table>';
        document.getElementById('SumAreas').innerHTML = 'Sum of Areas of Sectors = ' + Math.round(SumArea*1000000000000)/1000000000000
                +'<br>(sum of values in last column)';

        document.getElementById('sumofareas').innerHTML = 'Sum of Areas of Sectors<br>' + Math.round(SumArea*1000000000000)/1000000000000;
        ...
        document.getElementById('storge22').value = text55.substring(1);;
        document.getElementById('storge33').value = text66.substring(1);;
        document.getElementById('storge44').value = text11;
