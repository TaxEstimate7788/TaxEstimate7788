...
//alert('gggggg.....'+rdata );        
 ...
 ////////////////////
     ...
     function draw() {
        ...
        //var showalltangentlines = true;
        //if (showalltangentlines) {//show all tangent lines
        //    context.clearRect(0, 0, canvas.width, canvas.height);
        //}
        ...
        //plot point(s) of tangency

        var points = document.getElementById('tangencypoints').value;//pointtangency;        
        var lines = document.getElementById('tangentlinedata').value;//tangentlineequations;

        var case1 = (points.trim() != '' && points.indexOf('###') > -1) == true;
        var case2 = (points.trim() != '' && points.indexOf('###') == -1) == true;
        //var case3 = (points.trim() == '') == true;

        if (case1) {
            var datapoints = points.split('###');
            var datalines = lines.split('###');
        }
        else if (case2) {
        ...
        for (var i = 0; i < numOfPoints; i++) {
                ...
                var dataTangent2 = datalines[i].split(';;;');
...
function insertAtCaret(element, text, val1, val2, val3, val4) {
   ...
   function Textboxongrid22BB(casenum) {
    ...
    function clearcanvas(){
        ...
        document.getElementById('tangencypoints').value = '';
    ...
    function deltachange(casenum) {
        ...
        //Cvalue = FindValue(ProcessInput(Cvalue.toString()));
        var stepsize = document.getElementById('deltaXincrement').value;
        stepsize = FindValue(ProcessInput(stepsize.toString()));
        ...
        var equationtangentline = slopetangentline2 + '(x - (' + x1 + ')) + (' + y1 +')';
    ...
    function findtangentline() {
        ...
        if (Cvalue.indexOf(',') == -1 && Cvalue.indexOf('to') == -1) {
            var Carray = [Cvalue];
        }
        else if(Cvalue.indexOf('to') > -1  && Cvalue.indexOf('step') > -1){
            var tt = Cvalue.split('to');
            var tt2 = (tt[1]).split('step');
            var hh = '';
            var v1 = FindValue(ProcessInput(tt[0]));
            var v2 = FindValue(ProcessInput(tt2[0]));
            var v3 = FindValue(ProcessInput(tt2[1]));
        ...
        else if(Cvalue.indexOf('to') > -1  && Cvalue.indexOf('step') == -1){//stepsize = 1
            var tt = Cvalue.split('to');
            var hh = '';
            var v1 = FindValue(ProcessInput(tt[0]));
            var v2 = FindValue(ProcessInput(tt[1]));
            for (var kk = parseFloat(v1); kk <= parseFloat(v2); kk+=1) {
                hh += ',' + kk; 
            }
            hh = hh.substring(1);
            Carray = hh.split(',');            
        }
        else if (Cvalue.indexOf(',') > -1 && Cvalue.indexOf('to') == -1) {
            var Carray = Cvalue.split(',');
        }

        //find (c,f(c))
        var nn = Carray.length;
        //if (nn > 19) { nn = 19; }
        var tangentlineequations = '';
        var pointtangency = '';

        for (var kk = 0; kk < nn; kk++) {
            var cc = FindValue(ProcessInput(Carray[kk]));
            var gg = 19 - kk;
            //find (c,f(c))
            var temp1 = replaceAll(InputExpression, "x", "(" + cc.toString() + ")");
            var fc = FindValue(temp1);
            if (IsNumericc(fc) == false || fc == null) {
                fc = NaN;
            }
            fc = Math.round(fc * 1000000000000) / 1000000000000;
            //fc = ToDecimal2(fc.toString());

            if (isNaN(fc) == false) {
                //document.getElementById('Point'+gg+'X').value = cc;
                //document.getElementById('Point'+gg+'Y').value = fc;
                pointtangency += '###' + cc + ';' + fc;
                //(x3, f(x3))
                var x3 = parseFloat(cc) + 0.01;
                var temp1 = replaceAll(InputExpression, "x", "(" + x3.toString() + ")");
                var y3 = FindValue(temp1);
                y3 = Math.round(y3 * 1000000000000) / 1000000000000;

                var slopetangentline = (parseFloat(y3) - parseFloat(fc)) / (parseFloat(x3) - parseFloat(cc));
                var equationtangentline = slopetangentline + '(x - (' + cc + ')) + (' + fc + ')';
                //document.getElementById('TextBoxForCartesianGraph'+gg).value = equationtangentline;    
                ...
                var temp2 = replaceAll(equationtangentline, "x", "(" + xtt2.toString() + ")");
                var ytt2 = FindValue(temp2);
                tangentlineequations += '###' + xtt1 + ';' + xtt2 + ';;;' + ytt1 + ';' + ytt2;
        ...
        pointtangency = pointtangency.substring(3); 
        tangentlineequations = tangentlineequations.substring(3);

        //alert('pointtangency.....' + pointtangency);
        //alert('tangentlineequations.....' + tangentlineequations);
        document.getElementById('tangencypoints').value = pointtangency;        
        document.getElementById('tangentlinedata').value = tangentlineequations;
        document.getElementById('checkboxCartesianPoints').checked = true;
        document.getElementById("pointsizeS").checked = true;
        GraphingCalculator();
        return;
